...
static __inline__ typeof(simd_double1(^)(simd_double1)) rescaler (simd_double1 old_min, simd_double1 old_max, simd_double1 new_min, simd_double1 new_max) {
    return ^ simd_double1 (simd_double1 value) {
        return (simd_double1)(value = (new_max - new_min) * (value - old_min) / (old_max - old_min) + new_min);
    };
}

static __inline__ typeof(simd_double1(^)(simd_double1)) distributor (simd_double1 mean, simd_double1 standard_deviation) {
    simd_double1 variance = ((2.f * 0.5f) / M_PI) * (asin(sin(((2.f * M_PI) / 2.f) * mean)));
    simd_double1 offset = (variance * 0.5f) * standard_deviation;
    typeof(simd_double1(^)(simd_double1)) rescale = rescaler((mean - offset), (mean + offset), 0.f, 1.f);
    return ^ (simd_double1 * offset_t, typeof(simd_double1(^(* restrict))(simd_double1)) rescale_t) {
        return ^ simd_double1 (simd_double1 time) {
            return (simd_double1)(time = ((*rescale_t)(time)));
        };
    }(&offset, &rescale);
}

static simd_double1 (^(^randomize)(void))(void) = ^{
...
static AVAudioSourceNodeRenderBlock (^audio_renderer)(unsigned long) = ^ AVAudioSourceNodeRenderBlock (unsigned long frame_count) {
    ...
    simd_double1 (^frequency_distributor)(simd_double1) = distribute(0.75f, 0.125f);
    random_generator random_frequency_generator = generate_random(randomize())(frequency_distributor)(rescale);
    static simd_double1 frequency_random[3];
    ...
    // To-Do: generate a random value for the split frame at element 0
    //        use gaussian_distribution to generate the split frame at element[1]:
    //            value = element[0];
    ...
    return ^ OSStatus (BOOL *isSilence, const AudioTimeStamp *timestamp, AVAudioFrameCount frameCount, AudioBufferList *outputData) {
        for (AVAudioFramePosition frame = 0; frame < frameCount; frame++) {
            ({
                ...
                frequency_random[0] = gaussian_distribution(0.5, 0., random_float_between(0.1f, )
                    frequency_random[1] = (simd_double1)random_float_between(low_frequency, high_frequency);
                    frequency_random[2] = (simd_double1)random_float_between(low_frequency, high_frequency);
                    frequency_random[3] = (simd_double1)random_float_between(low_frequency, high_frequency);
                    frequencies = simd_matrix_from_rows(simd_make_double2(frequency_random[0],
                                                                          gaussian_distribution((simd_double1)random_float_between(low_frequency, high_frequency), (simd_double1)random_float_between(low_frequency, high_frequency), frequency_random[0])),
                                                        simd_make_double2(gaussian_distribution((simd_double1)random_float_between(low_frequency, high_frequency), (simd_double1)random_float_between(low_frequency, high_frequency), root_frequency))));
                    theta_increments = matrix_scale(phase_angular_unit, frequencies);
                    thetas = theta_increments;
                    split_frame = (simd_double1)random_float_between(0.125f, 0.875f);
                    distributed_split_frame = gaussian_distribution((simd_double1)random_float_between(0.125f, 0.875f), (simd_double1)random_float_between(0.125f, 0.875f), split_frame);
                    printf("\nsplit_frame == %f\ndistributed_split_frame == %f\n", split_frame, distributed_split_frame);
